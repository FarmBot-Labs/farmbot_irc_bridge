defmodule Farmbot.Jwt do
  ...
  def fetch_token!(username, password, server) do
    payload = Poison.encode!(%{user: %{email: username, password: password}})
    headers = ["content-type": "application/json"]

    HTTPoison.post!("#{server}/api/tokens", payload, headers)
    |> Map.fetch!(:body)
    |> Poison.decode!()
    |> Map.fetch!("token")
    |> Map.fetch!("encoded")
